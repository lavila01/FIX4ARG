 ...
 * @author mar
 */
public class QuotationsBymaJpaController implements Serializable {

    private EntityManagerFactory emf = null;
    public QuotationsBymaJpaController() {
...
//        this.emf = Persistence.createEntityManagerFactory("fix_mysql");
    ...
    public void create(QuotationsByma quotations) throws PreexistingEntityException, Exception {
    ...
    public void createIfnotExist(QuotationsByma quotations) throws NonexistentEntityException, Exception {
        EntityManager em = null;
        try {
            if (findQuotations(quotations.getSymbol()) != null) {
                edit(quotations);
    ...
    public void edit(QuotationsByma quotations) throws NonexistentEntityException, Exception {
        EntityManager em = null;
        try {
            em = getEntityManager();
            em.getTransaction().begin();
    ...
    public void destroy(String id) throws NonexistentEntityException {
        ...
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            QuotationsByma quotations;
            try {
                quotations = em.getReference(QuotationsByma.class, id);
    ...
    public List<QuotationsByma> findQuotationsEntities() {
        return findQuotationsEntities(true, -1, -1);
    }

    public List<QuotationsByma> findQuotationsEntities(int maxResults, int firstResult) {
        return findQuotationsEntities(false, maxResults, firstResult);
    }

    private List<QuotationsByma> findQuotationsEntities(boolean all, int maxResults, int firstResult) {
        EntityManager em = getEntityManager();
        try {
            CriteriaQuery cq = em.getCriteriaBuilder().createQuery();
            cq.select(cq.from(QuotationsByma.class));
    ...
    public QuotationsByma findQuotations(String id) {
        EntityManager em = getEntityManager();
        try {
            return em.find(QuotationsByma.class, id);
        } finally {
            em.close();
        }
    }
    ...
    public int getQuotationsCount() {
        ...
        try {
            CriteriaQuery cq = em.getCriteriaBuilder().createQuery();
            Root<QuotationsByma> rt = cq.from(QuotationsByma.class);
